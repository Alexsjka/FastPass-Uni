# FastPass-Uni

## Descripción del Proyecto
Este repositorio contiene el desarrollo del **caso práctico**, donde se realizó el análisis del sistema utilizando **historias de usuario, requerimientos y diagramas** para representar el funcionamiento del sistema.

El objetivo del proyecto es documentar y estructurar la solución del problema planteado mediante herramientas de modelado y documentación.

---

## Estructura del Proyecto

```
Proyecto
│
├── README.md
├── historias_usuario
   ├── historia_usuario_1.pdf
   ├── historia_usuario_2.pdf
   ├── historia_usuario_3.pdf
   └── historia_usuario_4.pdf

```

---

# Historias de Usuario

## Historia de Usuario 1

Como estudiante, quiero ver el menú disponible de cada punto de venta, 
para decidir qué comida pedir antes de ir a la cafetería.

📥 **Descargar Diagrama**

[Descargar Historia de Usuario 1](historias_usuario/historia_usuario_1.pdf)

---

## Historia de Usuario 2

Como estudiante, quiero realizar un pedido desde la aplicación, 
para evitar hacer filas durante el horario de almuerzo.

📥 **Descargar Diagrama**

[Descargar Historia de Usuario 2](historias_usuario/historia_usuario_2.pdf)

---

## Historia de Usuario 3

Como estudiante, quiero recibir una notificación cuando mi pedido esté listo, 
para pasar a recogerlo sin tener que esperar.

📥 **Descargar Diagrama**

[Descargar Historia de Usuario 3](historias_usuario/historia_usuario_3.pdf)

---

## Historia de Usuario 4

Como estudiante, quiero ver si el menú del día está disponible o agotado, 
para no perder tiempo en filas innecesarias.

📥 **Descargar Diagrama**

[Descargar Historia de Usuario 4](historias_usuario/historia_usuario_4.pdf)

---

# Requerimientos

## RF1 – Visualización del menú

El sistema debe permitir a los estudiantes consultar el menú disponible de 
los diferentes puntos de venta de la universidad.

---

## RF2 – Realización de pedidos

El sistema debe permitir a los estudiantes seleccionar productos del 
menú y realizar pedidos desde la aplicación.

---

## RF3 – Confirmación de pedido

El sistema debe confirmar al usuario que el pedido fue registrado correctamente.

---

## RF4 – Notificaciones

El sistema debe notificar al estudiante cuando su pedido esté listo para recoger.

---

# Licencia
Este proyecto fue desarrollado con **fines académicos**.

---

## Autores
- **Miño Cabrera Julian Alejandro**
- **Diaz Rodriguez Deivi Paul**
- **Bravo Gonzales Norman Davey**

---

